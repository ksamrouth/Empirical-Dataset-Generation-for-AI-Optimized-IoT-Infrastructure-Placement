...
% Last Modified by GUIDE v2.5 19-Mar-2024 11:04:41
...
grid_points = [x(:), y(:)];
if d0<=0.1
    point_size = 120 * (1 / (5*d0)); % Adjust the scaling factor as needed
elseif d0>0.1 && d0<=0.2
    point_size = 120 * (1 / (3*d0)) ;
elseif d0>0.2 && d0<=0.4
    point_size = 120 * (1 / (2*d0)) ;
elseif d0>0.4 && d0<0.7
    point_size = 120 * (1 / (1.5*d0)) ;
else
    point_size = 140;
end
% sensor_size=1.5*point_size;
scatter(grid_points(:, 1), grid_points(:, 2), point_size, 'filled','b');
hold on;
...
axis([min(grid_points(:,1))-d0 max(grid_points(:,1))+d0 min(grid_points(:,2))-d0 max(grid_points(:,2))+d0]);
grid on;
...
while true
    ...
    scatter(grid_points(min_sigma_index, 1), grid_points(min_sigma_index, 2), sensor_size, 'r', 'filled');
...
scatter(grid_points(k, 1), grid_points(k, 2), sensor_size, 'r', 'filled');
...
while true
    ...
    scatter(grid_points(max_index, 1), grid_points(max_index, 2), sensor_size, 'r', 'filled');
...
axes(handles.axes3);
cla(handles.axes3);
scatter(grid_points(:, 1), grid_points(:, 2), point_size, 'filled', 'b'); % Plot grid points in blue
hold on
for i = 1:size(grid_points, 1)
    text(grid_points(i, 1), grid_points(i, 2), num2str(i), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Color', 'yellow');
end
...
for i = 1:size(obstacle_points, 1)
    width = d0/4.5; % Width of the rectangle
    height = d0/3.5; % Height of the rectangle
